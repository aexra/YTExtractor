...
internal class MyMediaStream : Stream
{
    ...
    private static bool IsThrottled(IStreamInfo streamInfo) => !string.Equals(
            UrlEx.TryGetQueryParameterValue(streamInfo.Url, "ratebypass"),
            "yes",
            StringComparison.OrdinalIgnoreCase
        );

    public MyMediaStream(HttpClient http, IStreamInfo streamInfo)
    {
        _http = http;
        ...
        // them all separately.
        _segmentLength = IsThrottled(streamInfo) ? 9_898_989 : streamInfo.Size.Bytes;
    }
